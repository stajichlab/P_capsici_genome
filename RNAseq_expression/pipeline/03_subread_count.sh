...
#SBATCH --time 2:00:00 --out logs/subread_count.%a.log
...
do
    OUTFILE=$OUTDIR/$SAMPLE.gsnap_reads.tab
    ...
    OUTFILE=$OUTDIR/$SAMPLE.gsnap_reads.nostrand.tab
    INFILE=$INDIR/$SAMPLE.$EXTENSION
    if [ ! -f $OUTFILE ]; then
    ...
    OUTFILE=$OUTDIR/$SAMPLE.gsnap_frags.tab
    INFILE=$INDIR/$SAMPLE.$EXTENSION
    if [ ! -f $OUTFILE ]; then
